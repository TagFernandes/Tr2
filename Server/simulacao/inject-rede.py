import http.client  # <- Biblioteca padrão (contém HTTPConnection e HTTPSConnection)
import json
import socket       # <- Usado para a exceção socket.timeout
import time
import random
...
HOST = "tr2-api.tagfernandes.online"
PATH = "/sensor"
...
def enviar_dados(data):
    ...
    try:
        ...
        if response.status == 200:
            print(f"[{now_str}] Dados enviados com sucesso (HTTPS): {data}")
            try:
    ...
    # 8. Tratar erros de conexão e timeout
        ...
        print(f"[{now_str}] ERRO: Nao foi possivel conectar ao servidor")
        print("      (ConnectionRefusedError) Verifique se o servidor esta rodando e acessivel.")
...
if __name__ == "__main__":
    ...
    print(f"Enviando dados para https://{HOST}{PATH} a cada {INTERVALO_SEGUNDOS / 60:.0f} minutos.")
